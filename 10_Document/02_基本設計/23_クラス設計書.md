...
struct RouteSetting: Codable, Identifiable {
    ...
    /// 「行き」の時刻表URLの配列
    var outboundTimetableURLs: [URL]
    
    /// 「帰り」の時刻表URLの配列
    var inboundTimetableURLs: [URL]
...
**役割**: スクレイピングによって取得した、バス1便の情報を表現するデータ構造です。
...
役割: ウェブスクレイピング処理を専門に担当します。

```swift
class ScraperService {
    // MARK: - Methods
    
    /// 複数のURLから時刻表データを非同期で取得し、BusScheduleの配列を返す
    /// - Parameter urls: スクレイピング対象のURL配列
    /// - Returns: BusScheduleの配列
    /// - Throws: ネットワークエラーやHTMLの解析エラー
    func fetchSchedules(from urls: [URL]) async throws -> [BusSchedule] {
        // (ここに実際の処理を実装)
    }
}
```
...
class ScheduleViewModel: ObservableObject {
    ...
    /// 初期化処理
    init(scraperService: ScraperService = ScraperService(), holidayService: HolidayService = HolidayService()) {
